...
export default function AdminDashboardPage() {
  ...
  const [selectedLevelForPayment, setSelectedLevelForPayment] = useState<AssessmentLevel | null>(null)
  const [unlockedLevelIds, setUnlockedLevelIds] = useState<Set<string>>(new Set())
  const [pageLoading, setPageLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  const { metrics, recentStaff, payment, loading: metricsLoading, refetch: refetchMetrics } = useAdminMetrics(true)
    ...
    try {
      ...
      if (response.ok) {
        ...
        const levelIds = new Set(
          data.unlockedLevels?.filter((u: any) => u.is_unlocked === true).map((u: any) => u.level_id) || [],
        )
    ...
    <div className="px-6 py-12 space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="space-y-2">
          ...
          <p className="text-slate-600">Manage staff access, assessments, and unlock training levels</p>
        </div>
      </motion.div>
        ...
        >
          <Card className="border-0 shadow-sm bg-slate-50">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-slate-700">Payment Information</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-semibold text-slate-900">
                    ${payment?.amount_due ? payment.amount_due.toString() : "0.00"}
                  </p>
                </div>
              </div>
            </CardContent>
      ...
      <motion.div initial="hidden" whileInView="visible" variants={containerVariants} viewport={{ once: true }}>
        <Card className="border-0 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Unlock levels to grant staff access to assessments</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          {[
            ...
            <Link key={idx} href={action.href}>
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.2 }}>
                <Button className="w-full bg-blue-600 hover:bg-blue-700 rounded-lg" size="lg">
                  {action.label}
                </Button>
              </motion.div>
            </Link>
      ...
      >
        <Card className="border-0 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Latest staff members to join the program</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {recentStaff && recentStaff.length > 0 ? (
                recentStaff.map((staff, idx) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-xs font-medium text-slate-900">{staff.department}</p>
                      <p className="text-xs text-slate-600">{new Date(staff.created_at).toLocaleDateString()}</p>
                    </div>
                  </motion.div>
                ))
              ) : (
                <p className="text-slate-500 text-sm">No staff members registered yet</p>
              )}
            </div>
          </CardContent>
        </Card>
