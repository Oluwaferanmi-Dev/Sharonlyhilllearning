...
import { ArrowLeft, CheckCircle2, XCircle, Clock, ChevronDown, ClipboardList } from "lucide-react"
...
export default function StaffDetailPage() {
  ...
  const [assessmentsByLevel, setAssessmentsByLevel] = useState<LevelAssessments[]>([])
  ...
  useEffect(() => {
    const fetchStaffDetails = async () => {
      try {
        ...
        const message = err instanceof Error ? err.message : "Failed to load staff details"
  ...
  const handleViewDetails = async (assessmentId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/staff/${staffId}/assessment-results?assessmentId=${assessmentId}`)
    ...
    return (
      <div className="px-6 py-12">
        <Card className="border-red-200 bg-red-50">
          ...
          <CardContent>
            <p className="text-red-800 mb-4">{error || "Staff member not found"}</p>
  ...
  const totalTopics = assessmentsByLevel.reduce((sum, level) => sum + level.topics.length, 0)
  const completedTopics = assessmentsByLevel.reduce(
    (sum, level) => sum + level.topics.filter((t) => t.status === "completed").length,
    0,
  )
  ...
  return (
    <div className="px-6 py-12 space-y-8">
      <div className="flex items-center gap-4">
        <Button variant="outline" onClick={() => router.push("/admin/staff")} className="bg-transparent">
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="font-medium text-slate-900">{new Date(staff.created_at).toLocaleDateString()}</p>
              </div>
            </div>
      ...
      >
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-3xl font-bold text-blue-600">{completedTopics}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="space-y-0 pb-2">
            ...
            <CardContent>
              <p className="text-3xl font-bold text-green-600">{passedTopics}</p>
            </CardContent>
          </Card>
        </div>
      </motion.div>
      ...
      >
        <h2 className="text-2xl font-bold text-slate-900 mb-6">Assessment Progress</h2>
        <div className="space-y-6">
          {assessmentsByLevel.length === 0 ? (
            <Card>
              <CardContent className="py-8">
                <p className="text-center text-slate-600">No assessments started yet</p>
              </CardContent>
            </Card>
          ) : (
            assessmentsByLevel.map((level, idx) => (
              <Card key={level.levelId}>
                <CardHeader>
                  ...
                  <CardDescription>
                    {level.topics.filter((t) => t.status === "completed").length} of {level.topics.length} topics
                    completed
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    {level.topics.map((topic, topicIdx) => (
                      ...
                      >
                        <div
                          ...
                          onClick={() => topic.status === "completed" && handleViewDetails(topic.id)}
                        >
                          ...
                          {topic.status === "completed" && (
                            <div className="text-right flex items-center gap-3">
                              <div>
                                <p className={`text-2xl font-bold ${topic.passed ? "text-green-600" : "text-red-600"}`}>
                                  {topic.score}%
                                </p>
                                <p
                                  className={`text-xs font-semibold ${topic.passed ? "text-green-700" : "text-red-700"}`}
                                >
                              ...
                              <ChevronDown
                                className={`w-5 h-5 text-slate-600 transition-transform ${
                                  expandedAssessment === topic.id ? "rotate-180" : ""
                                }`}
                              />
                            </div>
                          )}
                        ...
                        {expandedAssessment === topic.id && assessmentDetails[topic.id] && (
                          <Card className="ml-4 border-slate-300 bg-slate-50">
                            <CardHeader>
                              <CardTitle className="text-base">Question-by-Question Results</CardTitle>
                              <CardDescription>
                                {assessmentDetails[topic.id].assessment.correctCount} of{" "}
                                {assessmentDetails[topic.id].assessment.totalQuestions} correct
                              </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                              {assessmentDetails[topic.id].results.map((result, qIdx) => (
                                <div
                                  ...
                                  <div className="space-y-2 text-sm">
                                    {Object.entries(result.options).map(([key, value]) => (
                                      <div
                                        ...
                                        className={`p-2 rounded ${
                                          key === result.correctAnswer
                                            ...
                                            : key === result.studentAnswer && !result.isCorrect
                                      ...
                                      >
                                        ...
                                        {key === result.studentAnswer && !result.isCorrect && (
                                          <span className="ml-2 text-red-700">✗ Selected</span>
                                        )}
                                      </div>
                                    ))}
                                  </div>
