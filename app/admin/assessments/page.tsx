...
export default function AssessmentsPage() {
  ...
  // BUG FIX: createClient() has been moved inside loadData() below.
  useEffect(() => {
    const loadData = async () => {
      // BUG FIX: client created inside the async function — once per fetch,
      // not once per render.
    ...
    loadData();
  }, [toast]); // `supabase` is no longer a dependency — it's created inside loadData
  ...
  const handleUpdatePrice = async (levelId: string, newPrice: number) => {
    ...
    try {
      ...
      setLevels((prevLevels) =>
        prevLevels.map((l) =>
          l.id === levelId
            ? { ...l, price_per_token: level.price_per_token }
            : l,
        ),
      );
    ...
    <div className="px-6 py-12 space-y-8">
      ...
      >
        ...
        <p className="text-slate-600">
          View and edit assessment levels, topics, and questions
        </p>
      </motion.div>
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div>
                        ...
                        <CardDescription className="flex items-center gap-2">
                          <span>
                            {levelTopics.length}{" "}
                            {levelTopics.length === 1 ? "topic" : "topics"}
                          </span>
                          ...
                          {editingLevelId === level.id ? (
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleUpdatePrice(level.id, editingPrice ?? 0)
                                }
        ...
        <Card className="border-yellow-200 bg-yellow-50">
          <CardContent className="pt-6">
            <p className="text-yellow-900">
              No assessment levels found. Please contact an administrator.
            </p>
          </CardContent>
        </Card>
      )}
