...
export async function PUT(
  ...
  try {
    ...
    if (body.price_per_token !== undefined) {
      const priceValidation = priceUpdateSchema.safeParse({
...
// BUG FIX: params type changed from { params: { levelId: string } } to
// { params: Promise<{ levelId: string }> } to match Next.js 15's runtime
// behaviour and the PUT handler above it.
export async function GET(
  ...
  try {
    ...
    const { data: level, error } = await adminClient
      .from("assessment_levels")
      .select(
        "id, name, order_index, description, price_per_token, price_currency",
      )
      .eq("id", levelId)
      .maybeSingle();
