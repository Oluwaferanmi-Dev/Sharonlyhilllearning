...
export async function POST(request: NextRequest) {
  try {
    ...
    if (adminCheckError) {
      ...
      return NextResponse.json({ error: "Database error while checking existing admin" }, { status: 500 })
    }

    if (existingAdmin) {
      console.log("[v0] Admin already exists, setup blocked")
    ...
    if (!profile) {
      ...
      if (profileError) {
        console.error("[v0] Manual profile creation error:", profileError.message)
