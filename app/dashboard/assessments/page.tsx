  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Get all levels
        const { data: levelsData } = await supabase.from("assessment_levels").select("*").order("order_index")
        ...
        const { data: unlocksData } = await supabase.from("level_unlocks").select("level_id, is_unlocked")
        ...
        const hasSeenPreAssessment = localStorage.getItem("seen_pre_assessment")
  ...
  return (
    <div className="px-6 py-12 space-y-8">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="bg-white/80 backdrop-blur rounded-xl p-6 shadow-sm border border-blue-100 space-y-4">
                <p className="text-base leading-relaxed text-slate-800">
                  <span className="font-semibold text-blue-900">Pre-assessment testing is a critical first step</span>{" "}
                  in preparing EDOHERMA's workforce to regulate safely and effectively against Joint Commission-style
                  standards across all domains. This approach allows each participant to see clearly which topics are
                  being assessed and how those topics map directly to the training modules and regulatory
                ...
                <div className="flex items-start gap-3 bg-blue-50 rounded-lg p-4 border border-blue-200">
                  ...
                  <p className="text-sm leading-relaxed text-slate-700">
                    The pre-assessment results drive an <strong>individualized learning roadmap</strong>—placing staff
                    into beginner, intermediate, or advanced pathways—so that time and resources are focused on closing
                    the most important knowledge and practice gaps that impact{" "}
                    <strong>inspection quality, risk-based regulation</strong>, and overall health-system performance in
                    Edo State.
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">
                      Systematically gauging baseline knowledge across 15 compliance areas
                    </p>
                  </div>
                </div>
                  ...
                  {COMPLIANCE_DOMAINS.map((domain, index) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-slate-900 leading-tight">{domain.name}</p>
                      </div>
                    </motion.div>
                  ))}
                </div>
              ...
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-200 rounded-xl p-6 space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed">
                      EDOHERMA ensures that training is <strong>targeted rather than generic</strong>, providing each
                      user with a unique learning experience based on their demonstrated knowledge gaps and learning
                      pathway.
                    </p>
                    <div className="pt-2">
                      <Button
      ...
      <div className="space-y-2">
        <h1 className="text-3xl md:text-4xl font-bold text-slate-900">Assessment Levels</h1>
        <p className="text-slate-600 text-lg">
          Complete assessments to demonstrate your healthcare compliance knowledge and advance your career
        </p>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        {levels?.map((level, idx) => {
          const assessments = userAssessments?.filter((a) => a.level_id === level.id) || []
          ...
          return (
            ...
            >
              <Card className={isLocked ? "opacity-75 border-slate-200 bg-slate-50" : ""}>
                {isLocked && (
                ...
                <CardContent className="space-y-4">
                  {isLocked ? (
                    ...
                    <>
                      <div>
                        <p className="text-sm font-medium text-slate-700 mb-2">Your Progress</p>
                        <p className="text-sm text-slate-600">
                          {completedCount} of {totalTopics} topics completed
                        </p>
                      </div>
                      <Link href={`/dashboard/assessments/${level.id}`}>
