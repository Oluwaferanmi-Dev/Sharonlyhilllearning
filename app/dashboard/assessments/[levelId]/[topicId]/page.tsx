...
export default function QuizPage() {
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      try {
        // Get topic
        const { data: topicData } = await supabase.from("assessment_topics").select("*").eq("id", topicId).single()

        setTopic(topicData)

        // Get questions
        const { data: questionsData } = await supabase.from("quiz_questions").select("*").eq("topic_id", topicId)
  ...
  if (showResults) {
    return (
      <div className="px-6 py-12 max-w-2xl mx-auto">
        <Card className={score >= 70 ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}>
          <CardHeader className="text-center">
            <CardTitle className={score >= 70 ? "text-green-900" : "text-red-900"}>Quiz Complete!</CardTitle>
            <CardDescription className={score >= 70 ? "text-green-800" : "text-red-800"}>{topic?.name}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="text-center">
              <p className={`text-5xl font-bold ${score >= 70 ? "text-green-600" : "text-red-600"}`}>{score}%</p>
              <p className={`text-lg font-semibold mt-2 ${score >= 70 ? "text-green-900" : "text-red-900"}`}>
            ...
            <div className="bg-white rounded-lg p-4 space-y-2">
              ...
              {score >= 70 && (
                <p className="text-sm text-green-700 font-semibold">Congratulations! You passed this assessment.</p>
              )}
              {score < 70 && (
                <p className="text-sm text-red-700 font-semibold">Please review the material and try again.</p>
              )}
            </div>
  ...
  return (
    <div className="px-6 py-12 max-w-2xl mx-auto space-y-8">
      {/* Back button */}
      <Button variant="outline" className="mb-4 bg-transparent" onClick={() => router.back()}>
        ← Back
      </Button>
      ...
      <div className="flex gap-4">
        ...
        {currentQuestionIndex < questions.length - 1 ? (
          ...
          <Button
            ...
            disabled={!answers[currentQuestion.id] || isSubmitting || Object.keys(answers).length !== questions.length}
            ...
            {questions.map((q, idx) => (
              <button
                ...
                className={`w-10 h-10 rounded-lg font-semibold transition-colors ${
                  answers[q.id] ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-600 hover:bg-slate-300"
