...
export default function AssessmentLevelPage({ params }: { params: { levelId: string } }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Get the level
        const { data: levelData } = await supabase.from("assessment_levels").select("*").eq("id", levelId).single()
    ...
    return (
      <div className="px-6 py-12">
        <Card className="border-yellow-200 bg-yellow-50">
          ...
          <CardContent>
            <p className="text-yellow-800 mb-4">This assessment level is locked. Please contact your administrator.</p>
  ...
  const topicOverviews: Record<string, { code: string; beginnerFocus: string }> = {
          ...
          <CardContent className="space-y-6">
            ...
            <div className="bg-white/80 backdrop-blur rounded-xl p-6 shadow-sm border border-blue-100 space-y-4">
              <p className="text-base leading-relaxed text-slate-800">
                <span className="font-semibold text-blue-900">Pre-assessment testing is a critical first step</span> in
                preparing EDOHERMA's workforce to regulate safely and effectively against Joint Commission-style
                standards across all domains. This approach allows each participant to see clearly which topics are
                being assessed and how those topics map directly to the training modules and regulatory responsibilities
                they hold.
              </p>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed">
                    EDOHERMA ensures that training is <strong>targeted rather than generic</strong>, providing each user
                    with a unique learning experience based on their demonstrated knowledge gaps and learning pathway.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="grid gap-4">
          {topics?.map((topic, idx) => {
            ...
            const overview = topicOverviews[topic.name] || {
              ...
              beginnerFocus: topic.description || "Assessment content for this topic",
            }
                ...
                <Card className={isCompleted ? "border-green-200 bg-green-50" : ""}>
                  <CardHeader className="pb-3">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-slate-700 leading-relaxed">{overview.beginnerFocus}</p>
                    </div>
                  </CardHeader>
