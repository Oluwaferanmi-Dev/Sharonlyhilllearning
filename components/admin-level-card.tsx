      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex-1">
          ...
          {!canUnlock && !isUnlocked && (
            <p className="text-xs text-slate-400 mt-2 font-medium">Not available for purchase</p>
          )}
        </div>
        <motion.div
          animate={{ scale: isUnlocked ? [1, 1.1, 1] : [1, 1.05, 1] }}
          transition={{ duration: 2, repeat: Number.POSITIVE_INFINITY }}
          className={`${isUnlocked ? "bg-green-100 p-2 rounded-lg" : "bg-slate-100 p-2 rounded-lg"}`}
        >
